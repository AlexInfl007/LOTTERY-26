...
### script-src 'self' 'unsafe-inline' 'unsafe-eval' 'wasm-unsafe-eval' chrome-extension: moz-extension:
- **'self'**: Allows scripts from the same origin
- **'unsafe-inline'**: Required for React/Vite development and inline scripts
- **'unsafe-eval'**: Required for compatibility with some wallet inpage/injected providers (MetaMask family).
- **'wasm-unsafe-eval'**: Required for WebAssembly execution (essential for ethers.js and other crypto libraries).
- **chrome-extension: / moz-extension:** Allows browser wallet extension scripts in Chromium/Firefox environments.
...
1. **Development vs Production**: The 'unsafe-inline' directive may be required for Vite development server and React Fast Refresh. For production, keep this enabled only if wallet-injection compatibility requires it; otherwise remove it.
